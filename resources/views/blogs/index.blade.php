...
@section('content')
    ...
    <div class="w-1/2">
        ...
        <p class="text-black-100">{{ $post->content }}</p>
    </div>

  <div class="flex space-x-2">
            <a href="{{ route('blogs.edit', $post->id) }}" class="bg-yellow-500 text-white px-4 py-1 rounded hover:bg-yellow-600">✏️ Edit</a>
            
            <form method="POST" action="{{ route('blogs.destroy', $post->id) }}">
                @csrf
                @method('DELETE')
                <button type="submit" onclick="return confirm('Are you sure?')" class="bg-red-600 text-white px-4 py-1 rounded hover:bg-red-700">
                    🗑️ Delete
                </button>
            </form>
            </div>
